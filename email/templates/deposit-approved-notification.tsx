...
export function DepositApprovedNotification({
  name,
  amount,
  subject,
  message,
  ...
  const dashboardUrl = `${url}/finance/deposit`;

  const statusStyles: Record<
    TransactionStatus,
    { color: string; bg: string; showCTA?: boolean }
  > = {
    pending: { color: "#b45309", bg: "bg-amber-100 text-amber-700", showCTA: true },
    completed: { color: "#047857", bg: "bg-green-100 text-green-700", showCTA: true },
    failed: { color: "#b91c1c", bg: "bg-red-100 text-red-700" },
    cancelled: { color: "#374151", bg: "bg-gray-100 text-gray-700" },
  } as const;
  ...
  const details = [
    ...
    { label: "Third-Party Transaction ID", value: thirdpartyTransactionId },
  ].filter((d) => d.value);
    ...
    <Wrapper title={subject} previewText={subject} url={url}>
      ...
      <Section className="px-8">
        <div className="border border-gray-200 rounded-lg bg-white shadow-sm">
          ...
          <Text className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${bg}`}>
      ...
      <Section className="pt-6 px-8">
        <Text className="text-sm text-gray-500">
          If you didn’t request this deposit, please contact our support team immediately.
        </Text>
      </Section>
    </Wrapper>
  );
